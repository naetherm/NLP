...
raw_documents = io.file('data/SMSSpamCollection', 'r').read().strip().split('\n')
documents = []
...
labels = np.array([1 if l == 'ham' else 0 for (t, l) in train_set])
## tokenizer
...
y_val = np.array([1 if l == 'ham' else 0 for (t, l) in test_set])
